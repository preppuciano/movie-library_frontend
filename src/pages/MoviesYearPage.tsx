...
import { getYearValueuByYearUrl } from "../utils/utils";

export default function MoviesYearPage() {
  const { year } = useParams();

  const { pageNumber = '1' } = useParams();

  const { data: response, error, isLoading } = useFetch<PaginationResponseInterface<MovieModel>>(() => getMoviesQuery({ page: Number(pageNumber), limit: 30, year: getYearValueuByYearUrl(year) }));
